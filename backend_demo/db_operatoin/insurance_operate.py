...
def add_insurance(dict):
    ...
    f = Insurance(name=dict['name'], product_id=dict['product_id'], amount_of_money=dict['amount_of_money'], flight_number=dict['flight_number'], status=dict['status'], date =datetime.datetime.now(),pic=dict['pic'] )
    db.session.add(f)
    db.session.commit()
    return f.id